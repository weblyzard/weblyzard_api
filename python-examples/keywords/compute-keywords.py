...
def read_corpus_files(path):
    ...
    for fname in glob(path):
        doc = {
            "id": RE_CONTENT_ID.search(fname).group(1),
            "body": RE_WHITESPACE.sub(
                " ", GzipFile(fname).read().replace("\n", " ").strip()
            ),
            "title": "",
            "format": "text/plain",
        }
...
if __name__ == "__main__":
    ...
    jesaja = Jesaja()
    jesaja.set_stoplist(
        STOPLIST_NAME, [stopword.strip() for stopword in GzipFile(STOPLIST_FILE)]
    )
    ...
    if not jesaja.has_corpus(matview_id=MATVIEW_NAME):
        ...
        while jesaja.rotate_shard(MATVIEW_NAME) == 0:
            ...
            jesaja.add_documents(
                MATVIEW_NAME,
                [doc.get_xml_document() for doc in list(xml_corpus_documents.values())],
            )
    ...
    for content_id, xml_document in list(xml_corpus_documents.items()):
        xml_document.add_attribute("keywords", "; ".join(result[str(content_id)]))
