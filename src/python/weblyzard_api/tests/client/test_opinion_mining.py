...
@pytest.fixture
def client():
    service_url = getenv("SENTIMENT_CLASSIFIER_URL", SERVICE_URL)
    if service_url is None:
        return
    client = OpinionClient(url=service_url)
    return client
...
class TestOpinionClient(object):
    ...
    def test_blocking_entity(self, client):
        ...
        result_without_annotations = client.get_polarity(
            content=xml_content_str, content_format="xml")
        assert result_without_annotations["polarity"] == 1.0

        # annotations provided: GeoEntities are blocked by default
        result_with_geo_entity = client.get_polarity(content=xml_content_str,
                                                     content_format="xml",
                                                     annotations=annotations)
        assert result_with_geo_entity["polarity"] == 0.0

        # non entity keywords are *not* blocked by default
        result_with_non_entity_keyword = client.get_polarity(
            content=xml_content_str, content_format="xml",
            annotations={"NonEntityKeyword": annotations["GeoEntity"]})
        ...
        result_with_non_entity_keyword_whitelisted = client.get_polarity(
            content=xml_content_str, content_format="xml",
            annotations={"NonEntityKeyword": annotations["GeoEntity"]},
            ignored_entity_regexp=r".*Entity.*")
