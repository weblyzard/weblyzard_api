...
class JeremiaTest(unittest.TestCase):
    ...
    def setUp(self):
        service_url = os.getenv("JEREMIA_NG_SERVICE_URL",
                                "http://jeremia.prod.i.weblyzard.net:8443")
    ...
    def test_dutch_string(self):
        ...
        result = self.client.submit_document(doc)
        tockens = set(
            [tocken["pos"] for tocken in result["partitions"]["TOKEN"]])
    ...
    def test_token_annotations(self):
        ...
        assert [text[slice(*char_range)] for char_range in
                zip(starts, ends)] == text.split()
        ...
        assert dependeny_labels == ["det", "nsubj", "ROOT", "advmod", "det",
                                    "dobj"]
