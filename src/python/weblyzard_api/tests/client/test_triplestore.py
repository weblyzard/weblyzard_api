...
class TriplestoreTestFuseki(unittest.TestCase):
    ...
    def test_fuseki_exists(self):
        assert (self.fuseki_wrapper.exists(
            uri="http://www.wikidata.org/entity/Q76"))

    def test_ask(self):
        assert (self.fuseki_wrapper.ask(
            query="ASK WHERE {{ wd:Q128660 wdt:P17 wd:Q40 }}"))
...
class TriplestoreTestBlazegraph(unittest.TestCase):
    ...
    def test_query(self):
        ...
        for result in self.blazegraph_wrapper.group_bindings(bindings):
            pprint(result)
            assert (
                all(k in result.keys() for k in ["uri", "label", "country"]))
            assert (all(value.split("@")[1] in ["de", "en"] for value in
                        result["label"]))
    ...
    def test_value_from_grouped_bindings(self):
        ...
        bindings = self.blazegraph_wrapper.run_query(query)
        for result in self.blazegraph_wrapper.group_bindings(bindings):
            assert (self.blazegraph_wrapper.get_value_from_grouped_bindings(
                result, field="label", language="de")
                    in "Öffentliche Einrichtung")
            assert (self.blazegraph_wrapper.get_value_from_grouped_bindings(
                result, field="label")
                    is None)  # no non-specified language value
            assert (self.blazegraph_wrapper.get_value_from_grouped_bindings(
                result, field="country", language="de")
    ...
    def test_exists(self):
        assert (self.blazegraph_wrapper.exists(
            uri="http://www.wikidata.org/entity/Q76"))

    def test_ask(self):
        assert (self.blazegraph_wrapper.ask(
            query="ASK WHERE {{ wd:Q128660 wdt:P17 wd:Q40 }}"))


class TriplestoreTestQlever(unittest.TestCase):
    # official qlever endpoint
    qlever_wrapper = QleverWrapper(
    ...
    def test_query(self):
        ...
        for result in self.qlever_wrapper.group_bindings(bindings):
            ...
            assert (
                all(k in result.keys() for k in ["uri", "label", "country"]))
            assert (
                all(value.split("@")[1] == "en" for value in result["label"]))

    def test_exists(self):
        assert (self.qlever_wrapper.exists(
            uri="http://www.wikidata.org/entity/Q76"))
...
def test_ts_wrapper_get_deduplicated_prefixes():
    ...
    assert (
                "PREFIX wd: <http://www.wikidata.org/entity/>" not in ts_wrapper.remove_duplicate_prefixes(
            query))
