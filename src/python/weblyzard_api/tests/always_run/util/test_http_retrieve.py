...
from weblyzard_api.util.http import DEFAULT_TIMEOUT, Retrieve, log, setdefaulttimeout
...
class TestHttpRetrieve(unittest.TestCase):
    ...
    def testRetrievalTimeout(self):
        ...
        with raises((timeout, urllib.error.URLError)):
            r = Retrieve(self.__class__.__name__, default_timeout=0.1).open(SLOW_URL)
            content = r.read()
            r.close()
