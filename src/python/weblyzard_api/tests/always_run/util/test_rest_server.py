...
# this is a better fit thematically for `weblyzard_api.tests.client`
# but placed here to make sure it is run automatically, as the client tests
# are not part of a pipeline due to external dependencies.


import unittest

from weblyzard_api.client import MultiRESTClient, RESTClient
...
class TestRESTClient(unittest.TestCase):
    ...
    def test_get_url(self):
        assert RESTClient.get_request_url(self.TEST_URL, "execute", "12").endswith(
            "/execute/12"
        )
        assert RESTClient.get_request_url(
            self.TEST_URL, "execute", "12", {"debug": True}
    ...
    def test_randomize_urls(self):
        ...
        very unlikely"""
        client = MultiRESTClient(service_urls="http://test.url", use_random_server=True)
        ...
        client = MultiRESTClient(service_urls=service_urls, use_random_server=True)
