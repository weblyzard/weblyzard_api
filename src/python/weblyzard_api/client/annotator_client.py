...
class Annotator:
    ...
    def _json_request(url, parameters):
        if parameters:
            req = urllib.request.Request(
                url, dumps(parameters), {"Content-Type": "application/json"}
            )
        else:
    ...
    def annotate_text(self, annotation_profile_name, documents):
        ...
        """
        return self._json_request(
            self.service_url + "/annotate_text/" + annotation_profile_name, documents
        )

    def annotate_xml(
        self, annotation_profile_name, documents, search_tag, is_cdata_encapsulated
    ):
        """
        @param annotation_profile_name: a dictionary of replacement patterns
            and the list of corresponding search patterns.
