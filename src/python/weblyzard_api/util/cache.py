...
def get_unique_temp_file(fname):
    return join(
        dirname(fname), "_%s-%s-%d" % (basename(fname), gethostname(), getpid())
    )
...
class DiskCached:
    ...
    def __init__(self, cache_dir, cache_nesting_level=0, cache_file_suffix=""):
        ...
        ::param cache_file_suffix:   optional suffix for cache files
        """
        self.cache = DiskCache(cache_dir, cache_nesting_level,
                               cache_file_suffix)
...
class MemoryCache(Cache):
    ...
    Caches arbitrary functions based on the function"s arguments (fetch) or
    on a user defined key (fetchObjectId)
    """
    ...
    def fetchObjectId(self, key, fetch_function, *args, **kargs):
        ...
        try:
            ...
            return self.fetch_with_fetch_function(key, fetch_function, *args,
                                                  **kargs)
    ...
    def garbage_collect_cache(self):
        ...
        (key, _) = sorted(
            list(self._usage.items()), key=itemgetter(1), reverse=True
        ).pop()
        del self._usage[key]
...
class RedisCache(Cache):
    ...
    def garbage_collect_cache(self):
        ...
        (key, _) = sorted(
            list(usage_dict.items()), key=itemgetter(1), reverse=True
        ).pop()
        del self._usage[key]
        del self._cacheData[key]
...
class HybridMemoryCached(MemoryCached):
    ...
    def fetch_with_fetch_function(self, key, fetch_function, *args, **kargs):
        self._dirty.add(key)
        return MemoryCached.fetch_with_fetch_function(
            self, key, fetch_function, *args, **kargs
        )
...
class HybridMemRedisCached(HybridMemoryCached):
    ...
    def __init__(
        ...
        HybridMemoryCached.__init__(
            self, max_cache_size=max_cache_size, key=key, group=group
        )
    ...
    def sync_upstream(self, priority: str = "local",
        ...
        if bulk_write:
            self.redis.hmset(
                self.key, {k: pickle.dumps(v) for k, v in upstream_data.items()}
            )
        else:
...
class HybridMemDiskCached(HybridMemoryCached):
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning(
                "No disk cached data found during initialization,"
