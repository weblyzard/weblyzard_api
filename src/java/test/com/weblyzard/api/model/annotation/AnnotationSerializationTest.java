...
public class AnnotationSerializationTest {
    ...
    public void testJsonSerialization() throws JsonProcessingException, IOException {
        Annotation testAnnotation = Annotation.build("http://dbpedia.org/resource/Augustine_of_Hippo")
                        .setPreferredName("Augustine of Hippo").setEntityType("PersonEntity").annotation()
                        .setSurfaceForm("Saint Augustine").setStart(0).setEnd(0).setSentence(1)
                        .setMd5sum(MD5Digest.fromText("Augustine of Hippo was an early Christian theologian."));

        serializeAndCompare(testAnnotation);

        testAnnotation.setEntityMetadata(ImmutableMap.of("rdf:type", ImmutableList.of("foaf:Person", "dbo:Person"),
